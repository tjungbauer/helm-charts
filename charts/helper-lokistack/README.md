...
|-----|------|---------|-------------|
| additionalAnnotations | string | {} | Additional annotations on the LokiStack resource (merged with Argo CD metadata via tpl.argocdMetadata). |
| additionalLabels | object | {} | Additional labels on the LokiStack resource. |
...
| networkPolicies | object | `{"ruleSet":"None"}` | NEW since Loki 6.4: The Operator can automatically create NetworkPolicies for the LokiStack components. The values can either be 'None' or 'RestrictIngressEgress' RestrictIngressEgress will deploy a set of Policies that restrict the communication to and from the Loki components. |
