...
**WARNING**: In order to create this backup, the CronJob must run a **privileged** pod on a **control-plane** node.
...
## Why privileged access and control-plane placement are required

OpenShift etcd backups are taken **on the control plane**, not from an ordinary application pod. The chart follows the approach documented by Red Hat: schedule work on a control-plane node and run `/usr/local/bin/cluster-backup.sh` in the host context via `oc debug node/...`.

That workflow requires capabilities a restricted pod cannot have:

1. **Control-plane scheduling** — the CronJob uses `nodeSelector: node-role.kubernetes.io/control-plane` so the backup logic runs where etcd and the backup script live.
2. **Host access** — the pod uses `hostNetwork`, `hostPID`, a hostPath mount of `/`, and `securityContext.privileged: true` so it can reach the node filesystem under `/host` and move files from `/host/home/core/backup`.
3. **`oc debug` on the local node** — the container runs `oc debug node/<local-node> -- bash -c 'chroot /host sudo -E /usr/local/bin/cluster-backup.sh ...'`, which needs permission to start debug pods and execute commands in the control-plane host namespace.
4. **OpenShift SCC** — item 5 below binds the ServiceAccount to `system:openshift:scc:privileged`. Without that SCC binding, the scheduler rejects the pod or the backup script cannot access the host paths it needs.

This is **not optional hardening debt**: a non-privileged, worker-node-only design cannot perform the supported control-plane etcd backup path. Treat the backup namespace, ServiceAccount, PVC, and privileged binding as highly sensitive infrastructure.
...
5. ClusterRoleBinding-Privileged: A binding that binds the ServiceAccount to the SCC `system:openshift:scc:privileged` (required for control-plane host access as described above).
